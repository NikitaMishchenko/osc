...
public:
          ...
          m_codomain(codomain)
    {
        if (m_domain.size() != m_codomain.size())
            throw std::range_error("cant construct Function with different domain and codomain sizes!");
    }
    ...
    virtual void emplace_back(const double domain, const double codomain)
    {
        m_domain.emplace_back(domain);
        m_codomain.emplace_back(codomain);
    }
...
inline Function sortFunction(Function func)
{
    ...
    valuesVect.reserve(func.size());

    for (int i = 0; i < func.size(); i++)
    {
        valuesVect.emplace_back(Values(func.getDomain(i), func.getCodomain(i)));
    }
    ...
    resultFnction.reserve(valuesVect.size());

    for (const auto &v : valuesVect)
    {
        resultFnction.emplace_back(v.x, v.y);
    }
